...
def resposta():
    R =f"""select Data, TarefaId, PerguntaOrdem, PerguntaDescricao,Conteudo 
    from [DW_Vista].[dbo].[FT_CHECKLIST_RESPOSTA_full]
    where ChecklistId = 'f982e19b-baa6-43f7-96ca-758cb98df93c'
    and  Tipo = 1
    AND PerguntaDescricao IS NOT NULL 
    AND CONVERT(varchar, Data, 103) LIKE '{ontem_fm}%' ORDER BY Data
    """
    Resposta = execute_select_with_retry(R, string_conexao)
    return Resposta 
...
def t_tarefa():
    ...
    tarefaid = execute_select_with_retry(comando, string_conexao)
    return tarefaid
 
def estrutura(id_est):
    est = f"""
        SELECT Id_Estrutura, Cliente, Descricao 
